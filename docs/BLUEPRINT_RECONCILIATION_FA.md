...
> **Casioplus یک محصول واحد با دو surface اجرایی است:** App برای control، عملیات و مصرف؛ Studio برای authoring و governance؛ Core/API برای canonical ownership، authorization و lifecycle مشترک.

| جزء                              | نقش در محصول واحد                                                                                                               | مرز قطعی                                                                  |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| App در `app.casioplus.com`       | ورود و account، Organization، Workspace، دعوت، دسترسی، command center، Work/Run، Review، Artifact، Memory view، usage و تنظیمات | به database، runtime داخلی یا credential دسترسی مستقیم ندارد.             |
| Studio در `studio.casioplus.com` | ساخت و ویرایش Flow، قرارداد ورودی/خروجی، policy، version، test، publish و governance                                            | clone رابط ابزار خارجی نیست و secret یا runtime credential نمایش نمی‌دهد. |
...
App نباید یک فهرست CRUD باشد. پس از ورود، کاربر باید بتواند از یک نمای عملیاتی بفهمد چه Workهایی فعال هستند، کدام Runها موفق یا ناموفق بوده‌اند، چه Reviewهایی منتظر تصمیم‌اند، وضعیت connectorها چیست و آخرین contextهای معتبر Memory کدام‌اند. این داده‌ها فقط زمانی در UI نمایش داده می‌شوند که از Core/API canonical آمده باشند.

Studio همین الگو را برای authoring به‌کار می‌گیرد: کاربر باید وضعیت Draft، validation، test run، policy، version و publish gate را در یک سطح قابل‌درک ببیند. وضعیت سبز فقط برای وضعیت تأییدشده و قابل‌اثبات مجاز است؛ وضعیت ناشناخته باید `not_configured`، اختلال باید `degraded` یا `error` و Flow آماده‌نشده باید با state واقعی خودش نمایش داده شود.

| الگوی تجربه      | App                                                      | Studio                                                               |
| ---------------- | -------------------------------------------------------- | -------------------------------------------------------------------- |
...
## ۳. رابطهٔ App و Studio

App و Studio دو hostname و دو deployment unit مستقل دارند، اما session، organization، workspace، Flow ownership، Run lifecycle، Memory policy و usage logic میان آن‌ها جدا نمی‌شود. ورود کاربر از App به Studio یک navigation بین دو surface همان محصول است، نه انتقال مالکیت یا ایجاد حساب جدید.

برای جلوگیری از hard-code شدن hostname در route، هر دو root loader اکنون `coreApiUrl`، `appUrl` و `studioUrl` را با `publicRuntimeConfigSchema` از `@casioplus/contracts` اعتبارسنجی می‌کنند. مقدارهای `CASIOPLUS_APP_URL` و `CASIOPLUS_STUDIO_URL` public هستند؛ هیچ token، cookie خام، secret یا tenant assertion از این مسیر منتقل نمی‌شود. احراز هویت و authorization همچنان باید توسط Core/API انجام شود.

| مسیر                              | قرارداد فعلی                                                                                       |
| --------------------------------- | -------------------------------------------------------------------------------------------------- |
| App → Studio                      | لینک public با `studioUrl` از root loader؛ بازشدن در surface Studio، بدون انتقال credential در URL |
| Studio → App                      | لینک public با `appUrl` از root loader؛ بازگشت به App همان محصول                                   |
...
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| App          | `/` command center؛ `work-items`، `work-items/:workItemId`، `runs`، `runs/:runId`، `review`، `memory`، `artifacts/:artifactId`، `integrations` و `settings` | command center پایه در `apps/app-web/app/routes/_index.tsx` پیاده شده؛ drill-downها در backlog مرحله‌ای هستند.                 |
| Studio       | `/` authoring center؛ `flows`، `flows/new`، `flows/:flowId`، `flows/:flowId/versions/:versionId`، `test-runs/:runId`، `memory`، `integrations` و `settings` | authoring/governance baseline در `apps/studio-web/app/routes/_index.tsx` پیاده شده؛ routeهای تفصیلی در backlog مرحله‌ای هستند. |
| App و Studio | root document، SSR، client hydration، stylesheet و public runtime config                                                                                    | پیاده‌شده در `root.tsx`، `entry.client.tsx`، `entry.server.tsx` و `remix.env.d.ts` هر دو surface                               |
...
```text
apps/app-web       → Remix App / command center / consumption
apps/studio-web    → Remix Studio / authoring / governance
...
| حوزه                                                                 | وضعیت                                                                         |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| App/Studio به‌عنوان یک محصول واحد                                    | implemented و در معماری، route shell و navigation ثبت شده است.                |
