# ADR-0002 — Remix به‌عنوان UI canonical برای Console و Forge
...
Casioplus یک محصول واحد با دو surface است: Console برای control و عملیات، و Forge برای authoring و governance. هر دو باید به Core/API مشترک متصل بمانند و نباید منطق canonical identity، authorization، tenant، lifecycle، memory یا usage را جداگانه مالک شوند.

## تصمیم

تمام UIهای Casioplus، شامل Console و Forge، فقط با Remix ساخته می‌شوند. Core/API با TypeScript/Node.js و PostgreSQL مسیر critical MVP باقی می‌ماند. Vite فقط compiler رسمی Remix Vite است؛ standalone Vite/React، static SPA server، `createRoot` مستقل، routing خارج از Remix و server entry مستقل از Remix برای UI canonical مجاز نیست.
...
فایل‌های UI به routeهای `apps/console-web/app/routes/_index.tsx` و `apps/forge-web/app/routes/_index.tsx` منتقل شدند. root، client entry، server entry، CSS import، NodeNext type declarations، package scripts، production Dockerfile و release manifest به قرارداد Remix تغییر کردند. اجرای `pnpm --filter @casioplus/console-web deploy --prod --legacy` و معادل Forge با موفقیت production package و `remix-serve` entry را تولید کرد.
