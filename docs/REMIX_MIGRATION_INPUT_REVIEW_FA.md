...
| ----------------- | ---------------------------------------------------- | --------------------------------------------------------------------------- |
| UI framework      | Console و Forge فقط Remix                            | پذیرفته و enforce‌شده با package scripts، route layout و topology validator |
...
| surface boundary  | دو Remix app در یک monorepo                          | پذیرفته؛ Console و Forge dependency و route boundary مستقل دارند            |
...
علاوه بر patch، topology validator در repository اکنون وجود routeهای Remix، dependencyهای لازم، scriptهای Remix، نبود dependencyهای مستقل و نبود مسیرهای `src/` و `index.html` را بررسی می‌کند. Dockerfileهای Console و Forge نیز باید `remix-serve` و `build/server/index.js` داشته باشند و نباید static server یا `/dist` را در runtime contract خود وارد کنند.

## ممیزی log

log پیوست موارد زیر را گزارش می‌کند: install موفق با pnpm 10.18.0، formatting موفق، typecheck موفق، هفت فایل و ۲۷ تست موفق، topology موفق و build موفق Core، Worker، Console و Forge. بخش build هر دو surface از `remix vite:build` و تولید هم‌زمان client/server خبر می‌دهد.
